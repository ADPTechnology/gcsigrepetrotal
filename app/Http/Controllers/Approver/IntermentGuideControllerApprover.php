...
class IntermentGuideControllerApprover extends Controller
{
       ...
       if($request->ajax())
        {
            ...
            {
                $guidesPending = IntermentGuide::where('stat_approved', 0)
                                                ->where('stat_rejected', 0)
                                                ->where(function ($q) use($user) {
                                                    $q->where('id_approvant', $user->id)
                                                        ->orWhereHas('previousApprovants', function ($q) use ($user) {
    ...
    public function update(Request $request, IntermentGuide $guide)
    {
        ...
        if ($guide->stat_approved == 0) {

            $guide->update([
                "stat_approved" => 1,
                "date_approved" => Carbon::now()->toDateTimeString(),
                "id_approvant" => $user->id
            ]);
        }
