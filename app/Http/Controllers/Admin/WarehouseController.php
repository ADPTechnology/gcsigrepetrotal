...
class WarehouseController extends Controller
{
    public function index(Request $request)
    {
        if($request->ajax())
        {
            if($request['table'] == 'warehouse'){
                    ...
                    $allWarehouses = DataTables::of($warehouses)
                                    ...
                                    ->editColumn('activity', function ($warehouse) {
                                        return config('parameters.activities')[$warehouse->activity] ?? '-';
                                    })
                                    ->editColumn('code', function ($warehouse) {
        ...
        $activities = config('parameters.activities');

        return view('principal.viewAdmin.warehouses.index', compact(
            'activities'
        ));
    }
    ...
    private function nameIsValid(string $name, ?Warehouse $currentWarehouse = null) : bool {
        $warehouse = Warehouse::where('name', trim($name))->select('id', 'name')->first();

        if ($currentWarehouse && ($warehouse == null || $currentWarehouse->id == $warehouse->id )) {
            return true;
        }
    ...
    {
        $name = $request['name'];
        $warehouse = $request['id'] ? Warehouse::find($request['id']) : null;

        return $this->nameIsValid($name, $warehouse) ? 'true' : 'false';
    }
